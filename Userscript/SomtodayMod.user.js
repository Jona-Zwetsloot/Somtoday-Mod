...
// SOMTODAY MOD (NON-MINIFIED VERSION)
...
window.addEventListener('load', function () { execute([onload]); }, false);

// Execute onload function after 1s if document is loaded
// This is to be sure that the mod didn't miss the load event
setTimeout(function() {
    if (!isloaded && (((!n(document.readyState)) && document.readyState == 'complete')) || (n(document.readyState))) {
        window.removeEventListener('load', function () { execute([onload]) }, false)
        onload();
    }
}, 1000);
...
function onload() {
    // Page has loaded or 1s has passed since script load
    isloaded = true;
    ...
    if (!n(id('somtoday-mod-active'))) {
        ...
        tn('body', 0).insertAdjacentHTML('beforeend', '<div id="somtoday-mod"><div id="somtoday-mod-active" style="display: none;"></div></div>');
    }
    ...
    function menu() {
        if (n(id('logo')) && (get('layout') == 2 || get('layout') == 3)) {
            id('main-menu').insertAdjacentHTML('afterbegin', '<center>' + getIcon('logo', null, colors[2], ' id="logo"') + '</center>');
        } else {
    ...
    function style() {
        ...
        // Menu
        tn('head', 0).insertAdjacentHTML('beforeend', '<style>@import url("https://fonts.googleapis.com/css2?family=Abhaya+Libre&family=Aleo&family=Archivo&family=Assistant&family=B612&family=Bebas+Neue&family=Black+Ops+One&family=Brawler&family=Cabin&family=Caladea&family=Cardo&family=Chivo&family=Crimson+Text&family=DM+Serif+Text&family=Enriqueta&family=Fira+Sans&family=Frank+Ruhl+Libre&family=Gabarito&family=Gelasio&family=IBM+Plex+Sans&family=Inconsolata&family=Inter&family=Josefin+Sans&family=Kanit&family=Karla&family=Lato&family=Libre+Baskerville&family=Libre+Franklin&family=Lora&family=Merriweather&family=Montserrat&family=Neuton&family=Noto+Serif&family=Nunito&family=Open+Sans&family=Oswald&family=Permanent+Marker&family=PT+Sans&family=PT+Serif&family=Playfair+Display:ital@1&family=Poppins&family=Poetsen+One&family=Quicksand&family=Raleway&family=Roboto&family=Roboto+Slab&family=Rubik&family=Rubik+Doodle+Shadow&family=Sedan+SC&family=Shadows+Into+Light&family=Single+Day&family=Source+Sans+3&family=Source+Serif+4:opsz@8..60&family=Spectral&family=Titillium+Web&family=Ubuntu&family=Work+Sans&display=swap");*,.ui-widget input,.ui-widget select,.ui-widget textarea,.ui-widget button,textarea{font-family:"' + get("fontname") + '","Open Sans","SOMFont",sans-serif;' + ((get("fontname") == "Bebas Neue" || get("fontname") == "Oswald") ? "letter-spacing:1px;" :"") + ' }#background-image-overlay{pointer-events:none;-webkit-user-select:none;user-select:none;z-index:-1;' + (get('layout') == 1 ? 'min-height:calc(100vh - 180px);width:70%;left:15%;top:160px;position:absolute;background:' + colors[8] + ';height:' + Math.max(id('master-panel').clientHeight,id('detail-panel-wrapper').clientHeight) + 'px;' :(get('layout') == 4 ? 'width:100%;left:0;top:0;position:fixed;background:' + colors[8] + ';height:100%;' :'')) + ' }#main-menu-wrapper{z-index:998;overflow:hidden;padding:0px;background:' + colors[0] + ';' + ((get('layout') == 1 || get('layout') == 4) ? 'position:absolute;left:0;top:80px;height:80px;' :'position:fixed;top:0;height:100%;width:120px;max-width:14vw;') + (get('layout') == 1 ? 'width:70%;border-top-left-radius:16px;border-top-right-radius:16px;margin-left:15%;' :(get('layout') == 2 ? 'left:0;' :(get('layout') == 3 ? 'right:0;' :'width:100%;'))) + '}h1 > #logo{height:35px;width:35px;margin-left:-50px;margin-top:-5px;position:absolute;}center > #logo{height:min(11vw,8vh);width:50%;padding:10px 0;}#main-menu{width:100%;height:100%;overflow-y:auto;}#main-menu div a{border-radius:0;background:transparent;color:white;max-height:14vw;' + ((get('layout') == 2 || get('layout') == 3) ? 'width:100%;height:' + (id('main-menu').getElementsByTagName('a').length > 8 ? '95px' :'100px;') :(get('layout') == 1 ? 'width:140px;height:80px;' :'width:150px;height:80px;')) + ' padding:0;margin:0;transition:background 0.3s ease;display:inline-block;}#main-menu div a svg{transition:transform 0.3s ease;}#main-menu div a.active svg,#main-menu div a:hover svg{transform:scale(1.1);}#main-menu div a.active,#main-menu div a:hover,#main-menu div a:focus{background:' + ((get('layout') == 1 || get('layout') == 4) ? colors[1] :(get('layout') == 2 ? 'linear-gradient(-90deg,' + colors[0] + ' 0%,' + colors[1] + ' 40%,' + colors[1] + ' 100%)' :'linear-gradient(90deg,' + colors[0] + ' 0%,' + colors[1] + ' 40%,' + colors[1] + ' 100%)')) + ' !important;color:white !important;}div.profile-img{background:' + colors[1] + ';}.profile-img{line-height:50px;border-radius:50%;overflow:hidden;-webkit-user-select:none;user-select:none;object-fit:cover;color:' + colors[2] + ';width:50px;height:50px;font-size:25px;text-align:center;font-weight:700;}#profile-img{transition:filter 0.2s ease,background 0.2s ease,color 0.2s ease;z-index:10000;position:absolute;right:' + (get('layout') == 1 ? 'calc(15% + 30px)' :'40px') + ';top:13px;}.topmenusvg{height:30px;width:30px;position:absolute;top:23px;transition:filter 0.2s ease;}#header-wrapper{visibility:hidden;}#user{box-sizing:border-box;' + (get('layout') == 1 ? '' :'border-bottom:3px solid ' + colors[4]) + ';' + ((get('layout') == 1 || get('layout') == 4) ? 'width:100%;margin-left:0;' :(get('layout') == 2 ? 'margin-left:min(14vw,120px);width:calc(100% - min(14vw,120px) - 6px);' :'margin-right:min(14vw,120px);margin-left:0;width:calc(100% - min(14vw,120px) - 6px);')) + ' visibility:visible;position:' + (get("bools").charAt(3) == "0" ? "absolute" :"fixed") + ';top:' + ((get('layout') == 1 && get('bools').charAt(3) == "0") ? '-20px' :'0') + ';left:0;z-index:1000;background:' + colors[12] + ';height:80px;}#user a{opacity:1 !important;}#user a:hover div#profile-img,div#profile-img:hover,#user a:focus div#profile-img{background:' + colors[7] + ';color:white;}#user a:hover img, #user a:hover svg,#user a:focus svg{filter:brightness(140%);}#user img:first-child{display:none;}#page-title{position:absolute;left:' + (get('layout') == 1 ? 'calc(15% + 70px)' :(get('layout') == 4 ? '80px' :'35px')) + ';top:25px;}#inbox-counter{position:absolute;right:' + (get('layout') == 1 ? 'calc(15% + 130px)' :'140px') + ';top:0;z-index:100000;width:fit-content !important;height:unset !important;min-width:24px;}#main-menu-wrapper label.MainMenuIcons:before{width:100%;font-size:26px;text-align:center;}.famenu,#main-menu-wrapper label.MainMenuIcons{display:block;width:100%;height:35%;padding-top:' + ((get("bools").charAt(1) == "0" && get('layout') == 1) ? '15%' :((get("bools").charAt(1) == "0" && get('layout') == 4) ? '12%' :'26%')) + ';}.famenu.tag,#main-menu-wrapper.tag label.MainMenuIcons{padding-top:' + ((get('layout') == 1 || get('layout') == 4) ? '15px' :'16%') + ';}#main-menu p,#main-menu q{font-size:15px;font-weight:700;width:100%;padding:10px 0;text-align:center;color:' + colors[2] + ';}</style>');
        // Calendar
        tn('head', 0).insertAdjacentHTML('beforeend', '<style>#calendar,#calendar *{box-sizing:border-box;}ul{list-style-type:none;}#calendar{position:absolute;top:0;right:0;' + ((get('layout') == 1 || get('layout') == 4) ? 'padding-top:' + (Math.round(66 / (get('zoom') / 100))).toString() + 'px;padding-left:20px;' :'padding-left:15%;') + ' height:calc((100vh - 80px) / ' + (get("zoom") / 100) + ');max-width:600px;z-index:1000;width:100%;background:' + (get('layout') == 1 ? 'transparent;border-top-right-radius:16px' :'linear-gradient(to right,rgba(0,0,0,0),' + colors[3] + ' 20%)') + ';-webkit-user-select:none;user-select:none;}.month{padding:25px 40px;width:100%;}.month ul{position:relative;margin:0;padding:0;}.month ul li{margin:0 5px;display:inline-block;color:' + colors[6] + ';font-size:20px;text-transform:uppercase;letter-spacing:3px;float:left;}#month{padding-left:15px;}#month,#year{padding-top:8px;}#weekdays{margin-top:40px;margin-bottom:0;padding:25px 40px;}#weekdays li{color:' + colors[11] + ';display:inline-block;text-align:center;}#days{padding:40px;margin:0;}#days li{position:relative;list-style-type:none;text-align:center;font-size:16px;color:' + colors[9] + ';aspect-ratio:1 / 1;padding-top:calc(50% - 18px);margin:8px;border-radius:50%;cursor:pointer;transition:0.2s background ease;}#days li:hover,#days li.selected{background:' + colors[5] + ';}#days li.empty{border:none;cursor:default;}#days li.active{background:' + colors[6] + ';color:white !important;font-weight:700;}#days li:after{content:"";position:absolute;display:none;border-radius:50%;margin-left:37.5%;margin-top:10%;width:25%;height:25%;}#days li.green:after{display:block;background:#39c380;}#days li.orange:after{display:block;background:#e8ad1c;}#days li.red:after{display:block;background:#f56558;}#days,#weekdays{display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr 1fr 1fr;grid-template-rows:1fr;}.empty{background:none !important;}@media all and (max-width:1300px){#calendar{padding-left:5%;background:linear-gradient(to right,rgba(0,0,0,0),' + colors[3] + ' 15%);}#days li{margin:15px 2px;padding-top:6px;}}</style>');
        // Media queries
        tn('head', 0).insertAdjacentHTML('beforeend', '<style>' + (get('layout') == 1 ? '@media (max-width:1440px){#page-title{left:calc(7.5% + 70px);}#profile-img{right:calc(7.5% + 30px);}svg#logout-btn{right:calc(7.5% + 170px);}svg#messages-btn{right:calc(7.5% + 110px);}#inbox-counter{right:calc(7.5% + 130px);}.bericht .r-content{max-width:calc(100% - 55px) !important;}.msgdetails1 > .pasfoto{width:40px !important;height:40px !important;margin-right:10px !important;line-height:38px;font-size:20px;}div#master-panel:has(.profileMaster){width:calc(85% / ' + (get('zoom') / 100) + ' - 235px) !important;}div#master-panel:has(.roosterMaster){width:calc(85% / ' + (get('zoom') / 100) + ') !important;}div#modactions{right:7.5%;}#main-menu-wrapper{margin-left:7.5% !important;width:85% !important;}#master-panel{left:7.5% !important;width:' + (48 / (get('zoom') / 100)) + '% !important;}#detail-panel-wrapper{width:calc(' + (35 / (get('zoom') / 100)) + '% - 15px) !important;right:7.5% !important;}#background-image-overlay{left:7.5% !important;width:85% !important;}}' :'') + ' @media (max-height:890px){#main-menu div a{height:90px;margin:0 !important;}.famenu{padding-top:22%;}.famenu.tag{padding-top:13%;}}@media (max-height:790px){#main-menu div a{height:80px;}.famenu{padding-top:20%;}.famenu.tag{padding-top:10%;}}@media (max-height:700px){#main-menu div a{height:70px;}.famenu{padding-top:17%;}.famenu.tag{padding-top:7%;}}@media (max-height:570px){#main-menu div a{height:60px;}.famenu{padding-top:14%;}.famenu.tag{padding-top:5%;}}@media (max-width:875px){#main-menu p{font-size:14px;}}@media (max-width:700px){#main-menu p{font-size:12px;}}@media (max-width:600px){#main-menu div a{margin:' + ((get('layout') == 4 || get('layout') == 1) ? '0' :'8px 0') + ';}}@media (max-width:500px){' + (get('layout') == 1 ? '' :'#inbox-counter{right:120px;}') + ' #profile-img{-webkit-user-select:none;user-select:none;right:30px;}svg#logout-btn{right:150px;}svg#messages-btn{right:100px;}#page-title{left:10px;}}@media (max-width:1024px){#content-wrapper.is-opendetailpanel #detail-panel-wrapper{top:80px;}}@media (max-width:1020px){.close-detailpanel{right:-5px !important;z-index:1;}.berichtenDetail .close-detailpanel,.activityDetail .close-detailpanel,.homeworkDetail .close-detailpanel{right:20px !important;}.homeworkDetail .blue.ribbon p{transform:translate(-35px,-13px);}.bericht .r-content{max-width:calc(100% - 110px) !important;}#header{margin-left:0;margin-right:0;}div#master-panel:has(.profileMaster){width:' + ((get('layout') == 1 || get('layout') == 4) ? 'calc(100% / ' + (get('zoom') / 100) + ' - 60px)' :'calc((100% - min(14vw,120px)) / ' + (get('zoom') / 100) + ' - 60px)') + ' !important;padding-right:0;}' + (get('layout') == 1 ? 'body div#master-panel:has(.roosterMaster){width:calc(100% / ' + (get('zoom') / 100) + ') !important;}' :'') + ' .roosterMaster .uurNummer{margin-left:0;}.hour.pauze{visibility:hidden;}' + (get('layout') == 1 ? '#profile-img{-webkit-user-select:none;user-select:none;right:50px;}svg#logout-btn{right:170px;}svg#messages-btn{right:120px;}#page-title{left:80px;}#inbox-counter{right:140px;}' :'') + ' #header .right{position:static;}#master-panel .date div p.date-day{margin-left:0;}.weekitems.eerste-kolom{margin-bottom:50px;}body #master-panel{left:' + (get('layout') == 2 ? 'min(120px,14vw)' :'0') + ' !important;width:' + ((get('layout') == 4 || get('layout') == 1) ? 'calc((100% / ' + (get('zoom') / 100) + '))' :'calc((100% - min(120px,14vw)) / ' + (get('zoom') / 100) + ')') + ' !important;}#background-image-overlay{left:0 !important;width:100% !important;}' + (get('layout') == 1 ? '#main-menu-wrapper{width:100% !important;margin-left:0 !important;}':'') + ' #content-wrapper #detail-panel-wrapper,#content-wrapper.is-opendetailpanel #master-panel{display:none !important;}#content-wrapper.is-opendetailpanel #detail-panel-wrapper,#content-wrapper #master-panel{display:block !important;top:' + ((get('layout') == 4 || get('layout') == 1) ? '160px' :'80px') + ' !important;}#content-wrapper.is-opendetailpanel #detail-panel{padding-top:20px;box-sizing:border-box;}.box.msgdetails2{margin-right:0;}.leermiddelen #content-wrapper.is-opendetailpanel #detail-panel-wrapper{min-height:60px;opacity:1 !important;}#content-wrapper.is-opendetailpanel #detail-panel-wrapper{overflow:hidden !important;height:fit-content;display:block;position:absolute;top:80px;z-index:10;width:calc((100% ' + ((get('layout') == 4 || get('layout') == 1) ? '' :'- min(120px,14vw)') + ') / ' + (get('zoom') / 100) + ' - 30px) !important;' + ((get('layout') == 2 || get('layout') == 3) ? (get('layout') == 3 ? 'right' :'left') + ':min(120px,14vw)' :'left:0') + ' !important;transform-origin:top left !important;right:0;padding:0 15px;}}@media all and (max-width:1700px){' + ((get('layout') == 4 || get('layout') == 1) ? '#main-menu div a{width:' + (100 / id('main-menu').getElementsByTagName('a').length) + '% !important;max-height:80px !important;}' :'') + ' .homeworkDetail .blue.ribbon p{visibility:hidden;}.homeworkDetail .blue.ribbon p a{visibility:visible;}}#logout-btn{right:' + (get('layout') == 1 ? 'calc(15% + 170px)' :'180px') + ';}#messages-btn{right:' + (get('layout') == 1 ? 'calc(15% + 110px)' :'120px') + ';}ul li label{color:' + colors[7] + ';font-size:14px;padding:5px;}input[type=text],input[type=number],#mod-message textarea{background:none;max-width:calc(100% - 44px);background:' + colors[12] + ';border:2px solid ' + colors[4] + ';color:' + colors[11] + ';transition:border 0.2s ease;border-radius:14px;padding:5px 10px;}input[type=text]:focus,input[type=number]:focus,#mod-message textarea:focus{outline:none;border:2px solid ' + colors[7] + ';}</style>');
        // Somtoday popups (roster popup and link popup)
        tn('head', 0).insertAdjacentHTML('beforeend', '<style>.fancybox-close,.ui-dialog .ui-dialog-titlebar-close{background:' + colors[0] + ';border-radius:50%;transition:0.2s background ease;animation:0.2s closebtnopacity ease;border:none;width:36px;height:36px;z-index:1000;overflow:hidden;}@keyframes closebtnopacity{0%{opacity:0;transform:scale(0);}100%{opacity:1;transform:scale(1);}}' + (get('layout') == 1 ? '.ui-dialog{transform:translateY(-20px);}' :'') + '.ui-dialog .ui-dialog-titlebar-close{margin-right:-20px;margin-top:-35px;}.fancybox-close:hover,.ui-dialog .ui-dialog-titlebar-close:hover{background:' + colors[1] + ';}.ui-icon.ui-icon-closethick{background-image:unset !important;position:relative;margin-top:-3px;margin-left:0;display:block;top:0;left:0;width:100%;height:100%;text-indent:0;}.fancybox-close:before,.ui-icon-closethick:before{content:"\u00D7";display:block;color:white;font-size:30px;margin:2px 8px;transition:transform 0.2s ease;}.fancybox-close:hover:before,.ui-dialog .ui-dialog-titlebar-close:hover .ui-icon-closethick:before{transform:scale(0.8);}.fancybox-wrap{overflow:visible !important;}.fancybox-skin,.box{background:' + colors[12] + ';padding:15px 20px !important;border-radius:12px;}.fancybox-skin{padding-right:0 !important;transition:0.3s box-shadow ease;box-shadow:' + (get('bools').charAt(0) == "1" ? '0 0 50px #555' :'0 0 50px #aaa') + ' !important;}.fancybox-skin *,.box *{color:' + colors[11] + ' !important;word-wrap:break-word;} .fancybox-inner,.fancybox-skin{width:665px !important;max-width:100%;}.fancybox-skin{max-width:calc(100% - 25px);}.fancybox-overlay{overflow:hidden !important;background:rgba(0,0,0,0.3);} .ui-widget.ui-widget-content,.ui-widget.ui-widget-content fieldset,fieldset input{color:' + colors[11] + ';border:none;}fieldset legend{margin-left:-2px;font-size:16px;font-weight:700;}fieldset label:last-of-type{display:none;}fieldset{border:none;padding-bottom:20px;}fieldset label input{margin-top:5px;}fieldset label{margin-top:10px;display:block;}fieldset input[type=submit],fieldset input[type=reset]{font-weight:700;cursor:pointer;padding-top:15px;background:none;}</style>');
        // Icon animations
    ...
    // Check if user should be redirected to the ELO
    function redirect() {
        if (location.href.indexOf("som.today") != -1) {
            if (get("bools") != null) {
                if (get("bools").charAt(4) == "0") {
                    return;
                }
            }
    ...
    function congratulations() {
        ...
        if (get("bools").charAt(6) == "1" && (year + "-" + (month + 1) + "-" + dayInt) != get("lastused")) {
            ...
            if (yeardifference > get('lastjubileum')) {
                congratstext = ["Bedankt, " + firstname + "!", "Je gebruikt Somtoday mod nu al " + yeardifference + " jaar. Bedankt!"];
    ...
    function panelStyles() {
        ...
        if ((get("layout") == 2 || get("layout") == 3) && n(cn("leermiddelenMaster", 0))) {
            if (id('detail-panel').clientHeight < 110) {
    ...
    function monthNavigation() {
        ...
        nexturl = 'https://elo.somtoday.nl/home/homework?datum=' + selectedday + '-' + (selectedmonth + 1) + '-' + selectedyear + '&windowsize=23';
        prevurl = 'https://elo.somtoday.nl/home/homework?datum=' + selectedday + '-' + (selectedmonth - 1) + '-' + selectedyear + '&windowsize=23';
        if (selectedmonth == 12) {
            nexturl = 'https://elo.somtoday.nl/home/homework?datum=' + selectedday + '-1-' + (selectedyear + 1) + '&windowsize=23';
        }
        if (selectedmonth == 1) {
            prevurl = 'https://elo.somtoday.nl/home/homework?datum=' + selectedday + '-12-' + (selectedyear - 1) + '&windowsize=23';
        }
        if (selectedday > 28 && selectedmonth == 3) {
            // Check for leap year
            if ((selectedyear / 4) == Math.round(selectedyear / 4)) {
                prevurl = 'https://elo.somtoday.nl/home/homework?datum=29-2-' + selectedyear + '&windowsize=23';
            } else {
                prevurl = 'https://elo.somtoday.nl/home/homework?datum=28-2-' + selectedyear + '&windowsize=23';
            }
        } else if (selectedday > 28 && selectedmonth == 1) {
            // Check for leap year
            if ((selectedyear / 4) == Math.round(selectedyear / 4)) {
                nexturl = 'https://elo.somtoday.nl/home/homework?datum=29-2-' + selectedyear + '&windowsize=23';
            } else {
                nexturl = 'https://elo.somtoday.nl/home/homework?datum=28-2-' + selectedyear + '&windowsize=23';
            }
        }
        // If the nextmonth and previousmonth buttons do not exist yet, insert them
        if (n(id("mod-nextmonth")) && !n(cn("d-next", 0))) {
            cn("d-next", 0).insertAdjacentHTML('afterend', '<a id="mod-nextmonth" href="' + nexturl + '">' + getIcon('forward', null, colors[6], 'style="height: 1.5em;"') + '</a>');
            bindTooltip('mod-nextmonth', 'Maand vooruit');
        }
        if (n(id("mod-prevmonth")) && !n(cn("d-prev", 0))) {
            cn("d-prev", 0).insertAdjacentHTML('beforebegin', '<a id="mod-prevmonth" href="' + prevurl + '">' + getIcon('backward', null, colors[6], 'style="height: 1.5em;"') + '</a>');
    ...
    function icons() {
        ...
        for (const element of cn('IconFont')) {
            if (element.classList.contains('icon-envelope-attach')) {
                setHTML(element, '<div style="position: absolute; height: 18px; width: 18px; background: ' + colors[7] + '; border-radius: 50%; margin-left: 18px; margin-top: -5px;">' + getIcon("paperclip", null, colors[12], 'style="width: 12px; height: 12px; margin-top: 3px"') + '</div>' + getIcon("envelope", null, colors[6]));
            } else if (element.classList.contains('icon-envelope-open-attach')) {
                setHTML(element, '<div style="position: absolute; height: 18px; width: 18px; background: ' + colors[7] + '; border-radius: 50%; margin-left: 18px; margin-top: -5px;">' + getIcon("paperclip", null, colors[12], 'style="width: 12px; height: 12px; margin-top: 3px"') + '</div>' + getIcon("envelope-open", null, colors[6]));
    ...
    function getBirthday() {
        ...
        for (const element of cn('label twopartfields')) {
            if (element.textContent.includes("Geboortedatum")) {
                if (!n(element.parentElement.parentElement.children[1])) {
                    if (!n(element.parentElement.parentElement.children[1].children[0])) {
                        if (!n(element.parentElement.parentElement.children[1].children[0].children[0])) {
            ...
            for (let i = 0; i < (namearray.length / 2); i++) {
                ...
                if (real != "") {
                    ...
                    for (const element of cn("r_content sub")) {
                        ...
                        if (!n(element.children[1])) {
                            if (!n(element.children[1].children[1])) {
    ...
    function profilePicture() {
        for (const element of cn("pasfoto")) {
            if (!n(element.src)) {
                if (element.src.indexOf('pasfoto_geen.jpg') != -1 && element.src.replace(new RegExp("(.+)(&antiCache.+)","g"),"$1") != profilepic) {
                ...
                if (!n(get("profilepic")) || get("bools").charAt(2) == "0") {
                    if (element.src.replace(new RegExp("(.+)(&antiCache.+)","g"),"$1") == profilepic) {
                        if (get("bools").charAt(2) == "0") {
                            element.insertAdjacentHTML('afterend', '<div class="pasfoto profile-img">' + initials + '</div>');
                            tryRemove(element);
                        }
                        else {
    ...
    // Hide/remove some elements if they shouldn't be visible anymore
    function hideElements() {
        // Hide modactions if page is not profile page
        if (n(cn("profileMaster", 0))) {
            tryRemove(id("modactions"));
            tryRemove(id("modsettingsfontscript"));
        }
        // Hide calendar, may be changed later
        hide(id('calendar'));
    }
        ...
        if (get("bools").charAt(11) == "0") {
            ...
            for (const element of cn('m-element')) {
                if (!n(element.parentNode)) {
                    if (!n(element.parentNode.parentNode)) {
                        if (!n(element.parentNode.parentNode.getElementsByClassName('date-month')[0])) {
                            ...
                            if (!n(id('month'))) {
                                if (id('month').innerHTML.charAt(0) == daymonth.charAt(0)) {
                                    if (!n(element.parentNode.parentNode.getElementsByTagName('span')[0])) {
                                        day = element.parentNode.parentNode.getElementsByTagName('span')[0].innerHTML;
                                        // Set id to day so calendar can reference to it with href="#id"
                                        element.parentNode.parentNode.getElementsByTagName('span')[0].id = day;
    ...
    function insertSettings() {
        ...
        cn("profileMaster", 0).insertAdjacentHTML('beforeend', '<div id="modsettings"><h3 class="category">Thema</h3><h3>Snelle thema\'s</h3><div id="theme-wrapper"></div>' +
            ...
            '<h3 class="category">Layout</h3><div id="layout-wrapper"><div class="layout-container' + (get('layout') == 1 ? ' layout-selected' : '') + '" id="layout-1"><div style="width: 120px; height: 25px; top: 5px; left: 30px;"><span>Menu</span></div><div style="width: 120px; height: 15px; top: 35px; left: 30px;"><span>Links</span></div><div style="width: 120px; height: 70px; top: 55px; left: 30px;"><span>Content</span></div><h3>Standaard</h3></div><div class="layout-container' + (get('layout') == 2 ? ' layout-selected' : '') + '" id="layout-2"><div style="width: 30px; height: 120px; top: 5px; left: 5px;"><span>Links</span></div><div style="width: 135px; height: 25px; right: 5px; top: 5px;"><span>Menu</span></div><div style="width: 135px; height: 90px; right: 5px; top: 35px;"><span>Content</span></div><h3>Sidebar links</h3></div><div class="layout-container' + (get('layout') == 3 ? ' layout-selected' : '') + '" id="layout-3"><div style="width: 135px; height: 25px; top: 5px; left: 5px;"><span>Menu</span></div><div style="width: 30px; height: 120px; top: 5px; right: 5px;"><span>Links</span></div><div style="width: 135px; height: 90px; left: 5px; top: 35px;"><span>Content</span></div><h3>Sidebar rechts</h3></div><div class="layout-container' + (get('layout') == 4 ? ' layout-selected' : '') + '" id="layout-4"><div style="width: 170px; height: 25px; top: 5px; left: 5px;"><span>Menu</span></div><div style="width: 170px; height: 15px; top: 35px; left: 5px;"><span>Links</span></div><div style="width: 170px; height: 70px; top: 55px; left: 5px;"><span>Content</span></div><h3>Breed</h3></div></div><h3 class="category">Achtergrond</h3>' +
        ...
        // Random background thanks to Lorem Picsum: https://picsum.photos
        id("randombackground").addEventListener("click", function() {
            ...
            if (!n(id('randombackground').previousElementSibling)) {
                ...
                if (!n(id("randombackground").previousElementSibling)) {
                    if (!n(id("randombackground").previousElementSibling.previousElementSibling)) {
                        if (!n(id("randombackground").previousElementSibling.previousElementSibling.getElementsByTagName('label')[0])) {
                            if (id("randombackground").previousElementSibling.previousElementSibling.getElementsByTagName('label')[0].classList.contains("mod-active")) {
    ...
    function modMessage(title, description, link1, link2, action1, action2) {
        ...
        id('somtoday-mod').insertAdjacentHTML('afterbegin', '<div id="mod-message" class="mod-msg-open"><center><div onclick="event.stopPropagation();"><h2>' + title + '</h2><p>' + description + '</p><a id="mod-message-action1" class="mod-message-button" tabindex="0">' + link1 + '</a>' + (n(link2) ? '' : '<a id="mod-message-action2" class="mod-message-button" tabindex="0">' + link2 + '</a>') + '</div></center></div>');
    ...
    // Check if user is visiting som.today
    if (location.href.indexOf("somtoday.nl") == -1) {
        execute([redirect]);
    }

    // User is visiting somtoday.nl
    else {
        ...
        // Execute functions
        execute([start, consoleMessage, generateColors, congratulations, menu, menuIcons, style]);

        // Execute pageUpdate after 0.05s to make sure everything is properly loaded
        setTimeout(function() {
            pageUpdate();
        }, 50);
        ...
        window.addEventListener('click', function(e) {
            e = e || window.event;
            if (!n((e.target || e.srcElement).parentElement)) {
                if (!n((e.target || e.srcElement).parentElement.parentElement)) {
                    if (!n((e.target || e.srcElement).parentElement.parentElement.parentElement)) {
        ...
        try {
            ...
            sendDebugData("Detailpanelobserver", e);
            console.error("Somtoday Mod ERROR\nFout bij het uitvoeren van Detailpanelobserver:")
            console.error(e);
        }

        // Assign masterpanelobserver to masterpanel
        try {
            ...
            sendDebugData("Masterpanelobserver", e);
            console.error("Somtoday Mod ERROR\nFout bij het uitvoeren van Masterpanelobserver:")
            console.error(e);
        }
    }
}