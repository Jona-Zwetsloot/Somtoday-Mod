...
function onload() {
    ...
    function menu() {
        ...
        if (logoutElement == null && !n(id('user').children[4])) {
            logoutElement = id('user').children[4];
        }
        if (profileElement == null && !n(id('user').children[1])) {
            profileElement = id('user').children[1];
        }
        if (messagesElement == null && !n(id('user').children[2])) {
            messagesElement = id('user').children[2];
        }
        if (!internalServerError && !n(id('user').children[4])) {
            setHTML(logoutElement, getIcon("right-from-bracket", "topmenusvg", colors[7], "id='logout-btn'"));
            logoutElement.title = "Uitloggen";
            let userPic = !n(get("profilepic"));
            setHTML(profileElement, (get("bools").charAt(2) == "1" && !(profilepic == null && !userPic)) ? "<img id='profile-img' class='profile-img' style='display: inline-block;' src='" + (userPic ? get("profilepic") : profilepic) + "'/>" : "<div id='profile-img' class='profile-img'>" + (n(initials) ? getIcon('user', null, null, 'style="margin-top:11px;"') : initials) + "</div>");
            setHTML(messagesElement, getIcon("envelope", "topmenusvg", colors[7], "id='messages-btn'"));
