...
function tryLoad() {
    ...
    if (!isloaded && (!n(tn('sl-root', 0)) || !n(id('master-panel')) || (!n(cn("stpanel--status", 0)) && !n(cn("errorTekst", 0)))) && (platform == "Userscript" || data != null)) {
...
function onload() {
    ...
    function newVersion() {
        ...
        tn('head', 0).insertAdjacentHTML('beforeend', '<style>sl-account-modal .content{position:relative;}#mod-setting-panel{position:absolute;background:white;top:0;left:0;width:100%;height:fit-content;padding:10px 30px; z-index:100;}</style>');
        tn('head', 0).insertAdjacentHTML('beforeend', '<style>#mod-background{position:fixed;left:0;width:100%;top:0;height:100%;object-fit:cover;z-index:-1;opacity: 0.3;}sl-studiewijzer-week:has(.datum.vandaag){background:rgba(255,255,255,0.65) !important;}sl-laatste-resultaat-item{background:' + colors[12] + ' !important;}sl-laatste-resultaat-item:hover{}</style>');
                ...
                if (real != "") {
                    ...
                    for (const element of cn("r_content sub")) {
                        if (!element.classList.contains('mod-nickname')) {
                            console.log('1' + nick);
                            if ((!n(element.children[1])) && !n(element.children[1].children[1]) || (!n(element.children[2]) && !n(element.children[2].children[1]))) {
                                console.log('2' + nick);
    ...
    function insertSettings() {
        ...
        const form = '<h3 class="category">Thema</h3><h3>Snelle thema\'s</h3><div id="theme-wrapper"></div>' +
            ...
            addSetting('Statistieken', 'Verzamel statistieken over het gebruik van de mod, zoals performance, FPS en meer.', 'bools16', 'checkbox', false) +
    ...
    function stats() {
        if (get("bools").charAt(16) == "1") {
            tn('body', 0).insertAdjacentHTML('beforeend', '<div id="stats" style="position: fixed; top: 10px; left: 10px; background: ' + colors[12] + '; border: 2px solid ' + colors[11] + '; padding: 10px 20px; z-index:10000;"><h3>Stats</h3><table><tbody><tr><td>Load</td><td><p>' + Math.round(initialLoad) + 'ms</p></td></tr><tr><td>Full load</td><td><p>' + Math.round(performance.now()) + 'ms</p></td></tr><tr><td>FPS</td><td><p id="fps">0</p></td></tr><tr><td>Screen</td><td><p>' + screen.width + 'x' + screen.height + '</p></td></tr><tr><td>Resolution</td><td><p id="resolution"></p></td></tr><tr><td>Pages visited</td><td><p id="pages">1</p></td></tr><tr><td>Online</td><td><p id="online">' + window.navigator.onLine + '</p></td></tr></tbody></table></div>')
        }
        function fpsMeter() {
        ...
        fpsMeter();
        window.addEventListener('resize', function () { setHTML(id('resolution'), window.innerWidth + 'x' + window.innerHeight); });
        let location = window.location.href;
        let pages = 1;
        setInterval(function() { if (location != window.location.href) { location = window.location.href; pages++; setHTML(id('pages'), pages) } }, 100);
        window.addEventListener('online', () => setHTML(id('online'), "true"));
        window.addEventListener('offline', () => setHTML(id('online'), "false"));
    }
