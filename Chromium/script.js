var data;
// GET DATA
chrome.storage.local.get(["background", "birthday", "blur", "bools", "enabled", "firstused", "fontname", "icon", "initials", "lastjubileum", "lastused", "layout", "nicknames", "primarycolor", "profilepic", "reload", "theme", "title", "transparency", "version", "zoom"]).then((result) => {
...
// Check if extension allows storage access
    ...
    function set(e, t) {
        ...
        try {
            ...
            // Extension is updating and storage connection is lost
            // Show updating message with spinner.
            if (!isUpdating) {
                modMessage('<center><h2>Update downloaden...</h2></center>', '<center><p>Somtoday Mod is aan het updaten naar versie ' + (version + 0.1) + '. Wacht alsjeblieft tot de update voltooid is.</p><span class="loader"></span></center><p>');
                isUpdating = true;
                // When the extension update is complete, a new tab will open which will show the user the update is complete
                // After that, if the user comes back to the Somtoday site (which will still have the old version loaded), reload the page
...
function addLoadListener() {
    if (window.location.origin.indexOf("somtoday.nl") == -1) {
        ...
        if (!n(tn('head', 0)) && n(id('transitions-disabled'))) {
...
function onload() {
    ...
    function getBirthday() {
        ...
        for (const element of cn('label twopartfields')) {
            if ((((element.textContent.includes("Geboortedatum")) && !n(element.parentElement.parentElement.children[1])) && !n(element.parentElement.parentElement.children[1].children[0])) && !n(element.parentElement.parentElement.children[1].children[0].children[0])) {
